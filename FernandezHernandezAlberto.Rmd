...
```{r, echo=FALSE}
library(formattable)
formattable(as.data.frame(matriz.correlaciones), list(
  area(row = 2, col = 3) ~ color_tile("#FFCCCB", "#FFCCCB"),
  area(row = 2, col = 12) ~ color_tile("#FFCCCB", "#FFCCCB"),
  area(row = 2, col = 13) ~ color_tile("#FFCCCB", "#FFCCCB"),
  area(row = 4, col = 13) ~ color_tile("#FFCCCB", "#FFCCCB")
))
```
...
formattable(round(as.data.frame(var$cos2), 2), list(
  ...
  area(row = c(2,3,4,12,13), col = 2) ~ color_tile("#0EBFE9", "#0EBFE9"),
  area(row = c(16), col = 3) ~ color_tile("#0EBFE9", "#0EBFE9")
)) %>% kable(caption = "Cosenos al cuadrado")
```
...
Para decidir el número de grupos, realizamos la agrupación con 3,4 y 5 _cluster_, comparando la calidad de cada modelo mediante los valores de anchura o _silhouette_ obtenidos:
...
for(x in c(3,4,5)) {
  set.seed(1234)
  RNGkind(sample.kind = "Rejection")
  km.x <- kmeans(provincias_ST, x)
  sil.x <- silhouette(km.x$cluster, dist(provincias_ST))
  cat("Numero de cluster: ", x, "\n")
  fviz_silhouette(sil.x, print.summary = T)
}
rm(km.x); rm(sil.x)
```
